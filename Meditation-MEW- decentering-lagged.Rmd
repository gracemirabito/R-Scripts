...
MRS.data <- EMA.subset[,c("GROUP", "ID","TIME2","Time.C","Decentering.C","PWB.C", "PhysicalActivity.C")]
#look at  data set
#head(MRS.data)
#tail(MRS.data)
...
MRS.data.redo <- MRS.data1 %>%                            # Add lagged column
  group_by(ID) %>%
  dplyr::mutate(PA.lag = dplyr::lead(PhysicalActivity.C, n = 1, default = NA), PWB.lag = dplyr::lead(PWB.C, n = 2, default = NA)) %>% 
  as.data.frame()
MRS.data.redo 
...
MRS.data.redo$Y.W <- MRS.data.redo$PWB.lag

#removing rumination and pwb to only leave the lagged variables in 
df = subset(MRS.data.redo, select = -c(PhysicalActivity.C, PWB.C) )


#melting the daya to use the dummy code double indicators
datalong.part1 <- reshape2:: melt(data=df,
                 id.vars=c("GROUP", "ID","TIME2","Time.C","Decentering.C","PA.lag","PWB.lag","X.M"),
...
model_lme_mod.part2 <- lme(fixed = z ~ -1 + 
                       ...
                       # y as outcome
                       dy + dy:Rumination.lag + dy:Decentering.C + dy:Time.C +
                       #  add in group as main effect and moderator
                       dy:GROUP.y + dy:Rumination.lag:GROUP.y + dy:Decentering.C:GROUP.y,
                  random = ~ -1  +  dm:Decentering.C + dy:Rumination.lag + dy:Decentering.C | ID,
                  weights = varIdent(form = ~ 1 | dvnum), #separate sigma^{2}_{e} for each outcome
...
model_lme_mod.part1 <- lme(fixed = z ~ -1 + 
                       ...
                       #  add in group as main effect and moderator
                       dy:GROUP.y + dy:Rumination.lag:GROUP.y + dy:Decentering.C:GROUP.y,
                  random = ~ -1  +  dm:Decentering.C + dy:Rumination.lag + dy:Decentering.C | ID, correlation= corAR1(),
                  weights = varIdent(form = ~ 1 | dvnum), #separate sigma^{2}_{e} for each outcome
                  data = datalong2.part1,
                  na.action = na.exclude)
                  #control = lmeControl(opt = "optim", maxIter = 200, msMaxIter = 200, niterEM = 50, msMaxEval = 400))
...
#PART 1 restructure for just mindfulness--> rumination --> stress
MRS.data2 <- EMA.c[,c("GROUP", "ID","TIME2","Time.C","Decentering.C","PWB.lag", "Rumination.lag","X","M","Y")]
#look at  data set
#head(MRS.data)
#tail(MRS.data)
datalong.c <- melt(data=MRS.data2,
               id.vars=c("GROUP", "ID","TIME2","Time.C","Decentering.C","PWB.lag","Rumination.lag","X"),
               na.rm=FALSE, variable.name="dv",
               value.name="z")
#look at updated data set
#head(datalong.part1)
...
model_lme.c <- lme(fixed = z ~ -1 + 
                             dm + dm:Decentering.C + dm:Time.C + #m as outcome
                             dy + dy:PWB.lag + dy:Decentering.C + dy:Time.C, #y as outcome
                  random = ~ -1  +  dm:Decentering.C + dy:PWB.lag + dy:Decentering.C | ID, correlation= corAR1(),
                  weights = varIdent(form = ~ 1 | dvnum), #separate sigma^{2}_{e} for each outcome
                  data = datalong.c,
                  na.action = na.exclude,
                  control = lmeControl(opt = "optim", maxIter = 200, msMaxIter = 200, niterEM = 50, msMaxEval = 400))
...
model_lme.c <- lme(fixed = z ~ -1 + 
                       ...
                       # y as outcome
                       dy + dy:PWB.lag + dy:Decentering.C + dy:Time.C +
                       #  add in group as main effect and moderator
                       dy:GROUP.y + dy:PWB.lag:GROUP.y + dy:Decentering.C:GROUP.y,
                  random = ~ -1  +  dm:Decentering.C + dy:PWB.lag + dy:Decentering.C | ID, correlation= corAR1(),
                  weights = varIdent(form = ~ 1 | dvnum), #separate sigma^{2}_{e} for each outcome
                  data = datalong.2.c,
                  na.action = na.exclude,
