...
MRS.data <- EMA.subset[,c("GROUP", "ID","TIME2","Time.C","Curiousity.C","PROMIS.C", "PhysicalActivity.C","X.M","M.R","Y.M")]
#look at  data set
#head(MRS.data)
#tail(MRS.data)
datalong.part1 <- melt(data=MRS.data,
               id.vars=c("GROUP", "ID","TIME2","Time.C","Curiousity.C","PROMIS.C","PhysicalActivity.C","X.M"),
...
#variables of interest for part 1 
vars <- c("TMS.C", "Curiousity.C", "Curiousity.C", "Stress.C", "PWB.C", "PROMIS.C", "PhysicalActivity.C")
#descriptives
describe(EMA.c[,vars])

#PART 1 restructure for just mindfulness--> rumination --> stress
MRS.data <- EMA.c[,c("GROUP", "ID","TIME2","Time.C","Curiousity.C","PROMIS.C", "PhysicalActivity.C","X","M","Y")]
#look at  data set
#head(MRS.data)
#tail(MRS.data)
datalong.c <- melt(data=MRS.data,
               id.vars=c("GROUP", "ID","TIME2","Time.C","Curiousity.C","PROMIS.C","PhysicalActivity.C","X"),
               na.rm=FALSE, variable.name="dv",
               value.name="z")
#look at updated data set
#head(datalong.part1)
...
model_lme.c <- lme(fixed = z ~ -1 + 
                             dm + dm:Curiousity.C + dm:Time.C + #m as outcome
                             dy + dy:PROMIS.C + dy:Curiousity.C + dy:Time.C, #y as outcome
                  random = ~ -1  +  dm:Curiousity.C + dy:PROMIS.C + dy:Curiousity.C | ID, 
                  weights = varIdent(form = ~ 1 | dvnum), #separate sigma^{2}_{e} for each outcome
                  data = datalong.c,
                  na.action = na.exclude,
                  control = lmeControl(opt = "optim", maxIter = 200, msMaxIter = 200, niterEM = 50, msMaxEval = 400))
...
model_lme.c <- lme(fixed = z ~ -1 + 
                       ...
                       # y as outcome
                       dy + dy:PROMIS.C + dy:Curiousity.C + dy:Time.C +
                       #  add in group as main effect and moderator
                       dy:GROUP.y + dy:PROMIS.C:GROUP.y + dy:Curiousity.C:GROUP.y,
                  random = ~ -1  +  dm:Curiousity.C + dy:PROMIS.C + dy:Curiousity.C | ID, 
                  weights = varIdent(form = ~ 1 | dvnum), #separate sigma^{2}_{e} for each outcome
                  data = datalong.2.c,
                  na.action = na.exclude,
