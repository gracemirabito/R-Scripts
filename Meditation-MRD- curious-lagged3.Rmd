...
MRS.data <- EMA.subset[,c("GROUP", "ID","TIME2","Time.C","Curiousity.C","PROMIS.C", "Rumination.C")]
#look at  data set
#head(MRS.data)
#tail(MRS.data)
...
datalong.part1 <- reshape2:: melt(data=df,
                 id.vars=c("GROUP", "ID","TIME2","Time.C","Curiousity.C","Rumination.lag","Depression.lag","X.M"),
...
#PART 1 restructure for just mindfulness--> rumination --> stress
MRS.data2 <- EMA.c[,c("GROUP", "ID","TIME2","Time.C","Curiousity.C","Depression.lag", "Rumination.lag","X","M","Y")]
#look at  data set
#head(MRS.data)
#tail(MRS.data)
datalong.c <- melt(data=MRS.data2,
               id.vars=c("GROUP", "ID","TIME2","Time.C","Curiousity.C","Depression.lag","Rumination.lag","X"),
               na.rm=FALSE, variable.name="dv",
               value.name="z")
#look at updated data set
#head(datalong.part1)
...
model_lme.c <- lme(fixed = z ~ -1 + 
                  ...
                  random = ~ -1  +  dm:Curiousity.C + dy:Depression.lag + dy:Curiousity.C | ID, 
                  weights = varIdent(form = ~ 1 | dvnum), #separate sigma^{2}_{e} for each outcome
                  data = datalong.2.c,
                  na.action = na.exclude,
                  control = lmeControl(opt = "optim", maxIter = 200, msMaxIter = 200, niterEM = 50, msMaxEval = 400))
