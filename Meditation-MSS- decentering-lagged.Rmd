...
MRS.data <- EMA.subset[,c("GROUP", "ID","TIME2","Time.C","Decentering.C","Stress.C", "Sleep.C")]
#look at  data set
#head(MRS.data)
#tail(MRS.data)
...
MRS.data.redo <- MRS.data1 %>%                            # Add lagged column
  group_by(ID) %>%
  dplyr::mutate(Sleep.lag = dplyr::lead(Sleep.C, n = 1, default = NA), Stress.lag = dplyr::lead(Stress.C, n = 2, default = NA)) %>% 
  as.data.frame()
MRS.data.redo 
...
df = subset(MRS.data.redo, select = -c(Sleep.C, Stress.C) )


#melting the daya to use the dummy code double indicators
datalong.part1 <- reshape2:: melt(data=df,
                 id.vars=c("GROUP", "ID","TIME2","Time.C","Decentering.C","Sleep.lag","Stress.lag","X.M"),
