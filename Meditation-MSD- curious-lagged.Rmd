...
MRS.data <- EMA.subset[,c("GROUP", "ID","TIME2","Time.C","Curiousity.C","PROMIS.C", "Sleep.C")]
#look at  data set
#head(MRS.data)
#tail(MRS.data)
...
datalong.part1 <- reshape2:: melt(data=df,
                 id.vars=c("GROUP", "ID","TIME2","Time.C","Curiousity.C","Sleep.lag","Depression.lag","X.M"),
...
model_lme.part1 <- lme(fixed = z ~ -1 + 
                             ...
                             dy + dy:Sleep.lag + dy:Curiousity.C + dy:Time.C, #y as outcome
                  random = ~ -1  +  dm:Curiousity.C + dy:Sleep.lag + dy:Curiousity.C | ID, correlation= corAR1(),
...
model_lme_mod.part2 <- lme(fixed = z ~ -1 + 
                       ...
                       # y as outcome
                       dy + dy:PA.lag + dy:Curiousity.C + dy:Time.C +
                       #  add in group as main effect and moderator
                       dy:GROUP.y + dy:PA.lag:GROUP.y + dy:Curiousity.C:GROUP.y,
                  random = ~ -1  +  dm:Curiousity.C + dy:PA.lag + dy:Curiousity.C | ID, 
                  weights = varIdent(form = ~ 1 | dvnum), #separate sigma^{2}_{e} for each outcome
...
model_lme_mod.part1 <- lme(fixed = z ~ -1 + 
                       ...
                       #  add in group as main effect and moderator
                       dy:GROUP.y + dy:PA.lag:GROUP.y + dy:Curiousity.C:GROUP.y,
                  random = ~ -1  +  dm:Curiousity.C + dy:PA.lag + dy:Curiousity.C | ID, correlation= corAR1(),
                  weights = varIdent(form = ~ 1 | dvnum), #separate sigma^{2}_{e} for each outcome
                  data = datalong2.part1,
                  na.action = na.exclude,
                  control = lmeControl(opt = "optim", maxIter = 200, msMaxIter = 200, niterEM = 50, msMaxEval = 400))
...
#PART 1 restructure for just mindfulness--> rumination --> stress
MRS.data2 <- EMA.c[,c("GROUP", "ID","TIME2","Time.C","Curiousity.C","Depression.lag", "PA.lag","X","M","Y")]
#look at  data set
#head(MRS.data)
#tail(MRS.data)
datalong.c <- melt(data=MRS.data2,
               id.vars=c("GROUP", "ID","TIME2","Time.C","Curiousity.C","Depression.lag","PA.lag","X"),
               na.rm=FALSE, variable.name="dv",
               value.name="z")
#look at updated data set
#head(datalong.part1)
...
model_lme.c <- lme(fixed = z ~ -1 + 
                             dm + dm:Curiousity.C + dm:Time.C + #m as outcome
                             dy + dy:Depression.lag + dy:Curiousity.C + dy:Time.C, #y as outcome
                  random = ~ -1  +  dm:Curiousity.C + dy:Depression.lag + dy:Curiousity.C | ID, correlation= corAR1(),
                  weights = varIdent(form = ~ 1 | dvnum), #separate sigma^{2}_{e} for each outcome
                  data = datalong.c,
                  na.action = na.exclude,
                  control = lmeControl(opt = "optim", maxIter = 200, msMaxIter = 200, niterEM = 50, msMaxEval = 400))
...
model_lme.c <- lme(fixed = z ~ -1 + 
                       ...
                       # y as outcome
                       dy + dy:Depression.lag + dy:Curiousity.C + dy:Time.C +
                       #  add in group as main effect and moderator
                       dy:GROUP.y + dy:Depression.lag:GROUP.y + dy:Curiousity.C:GROUP.y,
                  random = ~ -1  +  dm:Curiousity.C + dy:Depression.lag + dy:Curiousity.C | ID, correlation= corAR1(),
                  weights = varIdent(form = ~ 1 | dvnum), #separate sigma^{2}_{e} for each outcome
                  data = datalong.2.c,
                  na.action = na.exclude,
