...
vars <- c("TMS.C", "Curiousity.C", "Decentering.C", "Stress.C", "PWB.C", "PROMIS.C", "Sleep.C")
#descriptives
describe(EMA.subset[,vars])

#PART 1 restructure for just mindfulness--> rumination --> stress
MRS.data <- EMA.subset[,c("GROUP", "ID","TIME2","Time.C","Decentering.C","PROMIS.C", "Sleep.C")]
#look at  data set
#head(MRS.data)
#tail(MRS.data)
...
MRS.data.redo <- MRS.data1 %>%                            # Add lagged column
  group_by(ID) %>%
  dplyr::mutate(Sleep.lag = dplyr::lead(Sleep.C, n = 1, default = NA), Depression.lag = dplyr::lead(PROMIS.C, n = 2, default = NA)) %>% 
  as.data.frame()
MRS.data.redo 

#adding in the dummy coding variables with the lagged and centered variables required 

MRS.data.redo$X.M <- MRS.data.redo$Decentering.C
MRS.data.redo$M.R <- MRS.data.redo$Sleep.lag
MRS.data.redo$Y.W <- MRS.data.redo$Depression.lag

#removing rumination and pwb to only leave the lagged variables in 
df = subset(MRS.data.redo, select = -c(Sleep.C, PROMIS.C) )


#melting the daya to use the dummy code double indicators
datalong.part1 <- reshape2:: melt(data=df,
                 id.vars=c("GROUP", "ID","TIME2","Time.C","Decentering.C","Sleep.lag","Depression.lag","X.M"),
...
model_lme.part1 <- lme(fixed = z ~ -1 + 
                             ...
                             dy + dy:Sleep.lag + dy:Decentering.C + dy:Time.C, #y as outcome
                  random = ~ -1  +  dm:Decentering.C + dy:Sleep.lag + dy:Decentering.C | ID, correlation= corAR1(),
...
sig2_b <- as.numeric(VarCorr(model_lme.part1)["dy:PA.lag", "Variance"])
sig2_b
...
covajbj <- cortocov(r = as.numeric(VarCorr(model_lme.part1)["dy:PA.lag","Corr"]),
         var1 = as.numeric(VarCorr(model_lme.part1)["dm:Decentering.C","Variance"]),
         var2 = as.numeric(VarCorr(model_lme.part1)["dy:PA.lag","Variance"]))
...
model_lme_mod.part2 <- lme(fixed = z ~ -1 + 
                       ...
                       # y as outcome
                       dy + dy:PA.lag + dy:Decentering.C + dy:Time.C +
                       #  add in group as main effect and moderator
                       dy:GROUP.y + dy:PA.lag:GROUP.y + dy:Decentering.C:GROUP.y,
                  random = ~ -1  +  dm:Decentering.C + dy:PA.lag + dy:Decentering.C | ID,
                  weights = varIdent(form = ~ 1 | dvnum), #separate sigma^{2}_{e} for each outcome
...
model_lme_mod.part1 <- lme(fixed = z ~ -1 + 
                       ...
                       #  add in group as main effect and moderator
                       dy:GROUP.y + dy:PA.lag:GROUP.y + dy:Decentering.C:GROUP.y,
                  random = ~ -1  +  dm:Decentering.C + dy:PA.lag + dy:Decentering.C | ID, correlation= corAR1(),
                  weights = varIdent(form = ~ 1 | dvnum), #separate sigma^{2}_{e} for each outcome
                  data = datalong2.part1,
                  na.action = na.exclude,
                  control = lmeControl(opt = "optim", maxIter = 200, msMaxIter = 200, niterEM = 50, msMaxEval = 400))
...
a_mod
b_mod <- as.numeric(lme4::fixef(model_lme_mod.part1)["dy:PA.lag"])
b_mod
...
aXtreat_mod
bXtreat_mod <- as.numeric(lme4::fixef(model_lme_mod.part1)["GROUP.y:dy:PA.lag"])
bXtreat_mod
...
covajbj_mod <- cortocov(r = as.numeric(VarCorr(model_lme_mod.part1)["dy:PA.lag","Corr"]),
         var1 = as.numeric(VarCorr(model_lme_mod.part1)["dm:Decentering.C","Variance"]),
         var2 = as.numeric(VarCorr(model_lme_mod.part1)["dy:PA.lag","Variance"]))
covajbj_mod
```
...
#make x,y,m 
EMA.c <- df 
EMA.c$X <- df$Decentering.C
EMA.c$M <- df$Depression.lag
EMA.c$Y <- df$Rumination.lag


#PART 1 restructure for just mindfulness--> rumination --> stress
MRS.data2 <- EMA.c[,c("GROUP", "ID","TIME2","Time.C","Decentering.C","Depression.lag", "Rumination.lag","X","M","Y")]
#look at  data set
#head(MRS.data)
#tail(MRS.data)
datalong.c <- melt(data=MRS.data2,
               id.vars=c("GROUP", "ID","TIME2","Time.C","Decentering.C","Depression.lag","Rumination.lag","X"),
               na.rm=FALSE, variable.name="dv",
               value.name="z")
#look at updated data set
#head(datalong.part1)
...
model_lme.c <- lme(fixed = z ~ -1 + 
                             dm + dm:Decentering.C + dm:Time.C + #m as outcome
                             dy + dy:Depression.lag + dy:Decentering.C + dy:Time.C, #y as outcome
                  random = ~ -1  +  dm:Decentering.C + dy:Depression.lag + dy:Decentering.C | ID, correlation= corAR1(),
                  weights = varIdent(form = ~ 1 | dvnum), #separate sigma^{2}_{e} for each outcome
                  data = datalong.c,
                  na.action = na.exclude,
                  control = lmeControl(opt = "optim", maxIter = 200, msMaxIter = 200, niterEM = 50, msMaxEval = 400))
...
model_lme.c <- lme(fixed = z ~ -1 + 
                       ...
                       # y as outcome
                       dy + dy:Depression.lag + dy:Decentering.C + dy:Time.C +
                       #  add in group as main effect and moderator
                       dy:GROUP.y + dy:Depression.lag:GROUP.y + dy:Decentering.C:GROUP.y,
                  random = ~ -1  +  dm:Decentering.C + dy:Depression.lag + dy:Decentering.C | ID, correlation= corAR1(),
                  weights = varIdent(form = ~ 1 | dvnum), #separate sigma^{2}_{e} for each outcome
                  data = datalong.2.c,
                  na.action = na.exclude,
