...
MRS.data <- EMA.subset[,c("GROUP", "ID","TIME2","Time.C","Decentering.C","PWB.C", "Rumination.C")]
#look at  data set
#head(MRS.data)
#tail(MRS.data)
...
MRS.data.redo <- MRS.data1 %>%                            # Add lagged column
  group_by(ID) %>%
  dplyr::mutate(Rumination.lag = dplyr::lead(Rumination.C, n = 1, default = NA), PWB.lag = dplyr::lead(PWB.C, n = 2, default = NA)) %>% 
  as.data.frame()
MRS.data.redo 
...
df = subset(MRS.data.redo, select = -c(Rumination.C, PWB.C) )


#melting the daya to use the dummy code double indicators
datalong.part1 <- reshape2:: melt(data=df,
                 id.vars=c("GROUP", "ID","TIME2","Time.C","Decentering.C","Rumination.lag","PWB.lag","X.M"),
