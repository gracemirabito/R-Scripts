...
MRS.data <- EMA.subset[,c("GROUP", "ID","TIME2","Time.C","TMS.C","Stress.C", "Rumination.C","X.M","M.R","Y.S")]
#look at  data set
#head(MRS.data)
#tail(MRS.data)
datalong.part1 <- melt(data=MRS.data,
               id.vars=c("GROUP", "ID","TIME2","Time.C","TMS.C","Stress.C","Rumination.C","X.M"),
...
model_lme.part1 <- lme(fixed = z ~ -1 + 
                             dm + dm:TMS.C + dm:Time.C + #m as outcome
                             dy + dy:Rumination.C + dy:TMS.C + dy:Time.C, #y as outcome
                  random = ~ -1  +  dm:TMS.C + dy:Rumination.C + dy:TMS.C | ID, 
...
Effects on M (Rumination):
	-0.096 = effect of x –> m (a: Mindfulness predicting Rumination) ***
	-0.007 = effect of time –> m (time trend in Rumination) ***
Effects on Y (Stress):
	0.192 = effect of m –> y (b: Rumination predicting Stress) ***
	0.01 = effect of x –> y (c’: Mindfulness predicting Stress, after adjusting for Rumination) NOT SIG 
...
sig2_a <- as.numeric(VarCorr(model_lme.part1)["dm:TMS.C", "Variance"])
sig2_a
...
sig2_cprime <- as.numeric(VarCorr(model_lme.part1)["TMS.C:dy", "Variance"])
sig2_cprime
...
sig2_ey <- (1.708194 *as.numeric(VarCorr(model_lme.part1)["Residual", "StdDev"]))^2
sig2_ey
...
covajbj <- cortocov(r = as.numeric(VarCorr(model_lme.part1)["dy:Rumination.C","Corr"]),
         var1 = as.numeric(VarCorr(model_lme.part1)["dm:TMS.C","Variance"]),
...
model_lme_mod.part1 <- lme(fixed = z ~ -1 + 
                       # m as outcome
                       dm + dm:TMS.C + dm:Time.C + 
                       # add in group as main effect and moderator
                       dm:GROUP.y + dm:TMS.C:GROUP.y + 
                       
                       # y as outcome
                       dy + dy:Rumination.C + dy:TMS.C + dy:Time.C +
                       #  add in group as main effect and moderator
                       dy:GROUP.y + dy:Rumination.C:GROUP.y + dy:TMS.C:GROUP.y,
                  random = ~ -1  +  dm:TMS.C + dy:Rumination.C + dy:TMS.C | ID, 
                  weights = varIdent(form = ~ 1 | dvnum), #separate sigma^{2}_{e} for each outcome
...
* **`r round(lme4::fixef(model_lme_mod.part1)["dm:TMS.C"], 3)` = effect of x --> m (*a*: work stressors predicting work dissatisfaction across averaging conditions)**     
...
* **`r round(lme4::fixef(model_lme_mod.part1)["dm:TMS.C:GROUP.y"], 3)` = interaction of x and treatment --> m (moderation of *a*: differences in effect of work stressors predicting work dissatisfaction as a function of treatment )** 
...
* **`r round(lme4::fixef(model_lme_mod.part1)["TMS.C:dy"], 3)` = effect of x --> y (*c'*: work stressors predicting relationship dissatisfaction, after adjusting for work dissatisfaction, averaging across conditions)** 
...
* **`r round(lme4::fixef(model_lme_mod.part1)["TMS.C:GROUP.y:dy"], 3)` = interaction of x and treatment --> y (moderation of *c*: differences in effect of work stressors predicting relationship dissatisfaction as a function of treatment)** 
...
# mediation paths
a_mod <- as.numeric(lme4::fixef(model_lme_mod.part1)["dm:TMS.C"])
a_mod
b_mod <- as.numeric(lme4::fixef(model_lme_mod.part1)["dy:Rumination.C"])
b_mod
cprime_mod <- as.numeric(lme4::fixef(model_lme_mod.part1)["TMS.C:dy"])
cprime_mod

# moderation of mediation paths
aXtreat_mod <- as.numeric(lme4::fixef(model_lme_mod.part1)["dm:TMS.C:GROUP.y"])
aXtreat_mod
bXtreat_mod <- as.numeric(lme4::fixef(model_lme_mod.part1)["GROUP.y:dy:Rumination.C"])
bXtreat_mod
cprimeXtreat_mod <- as.numeric(lme4::fixef(model_lme_mod.part1)["TMS.C:GROUP.y:dy"])
cprimeXtreat_mod
```
...
covajbj_mod <- cortocov(r = as.numeric(VarCorr(model_lme_mod.part1)["dy:Rumination.C","Corr"]),
         var1 = as.numeric(VarCorr(model_lme_mod.part1)["dm:TMS.C","Variance"]),
...
ind_ranefs_treat$indirect <-
  # moderated a path
  (ind_ranefs_treat$dm.TMS.C + ind_ranefs_treat$GROUP * ind_ranefs_treat$dm.TMS.C.GROUP.y) *
  # moderated b path
...
#make x,y,m 
EMA.c <- EMA.subset
EMA.c$X <- EMA.subset$TMS.C
EMA.c$M <- EMA.subset$Stress.C
EMA.c$Y <- EMA.subset$Rumination.C
...
#PART 1 restructure for just mindfulness--> rumination --> stress
MRS.data <- EMA.c[,c("GROUP", "ID","TIME2","Time.C","TMS.C","Stress.C", "Rumination.C","X","M","Y")]
#look at  data set
#head(MRS.data)
#tail(MRS.data)
datalong.c <- melt(data=MRS.data,
               id.vars=c("GROUP", "ID","TIME2","Time.C","TMS.C","Stress.C","Rumination.C","X"),
               na.rm=FALSE, variable.name="dv",
               value.name="z")
#look at updated data set
#head(datalong.part1)
...
model_lme.c <- lme(fixed = z ~ -1 + 
                             dm + dm:TMS.C + dm:Time.C + #m as outcome
                             dy + dy:Stress.C + dy:TMS.C + dy:Time.C, #y as outcome
                  random = ~ -1  +  dm:TMS.C + dy:Stress.C + dy:TMS.C | ID, 
                  weights = varIdent(form = ~ 1 | dvnum), #separate sigma^{2}_{e} for each outcome
                  data = datalong.c,
                  na.action = na.exclude,
                  control = lmeControl(opt = "optim", maxIter = 200, msMaxIter = 200, niterEM = 50, msMaxEval = 400))
...
model_lme.c <- lme(fixed = z ~ -1 + 
                       ...
                       # y as outcome
                       dy + dy:Stress.C + dy:TMS.C + dy:Time.C +
                       #  add in group as main effect and moderator
                       dy:GROUP.y + dy:Stress.C:GROUP.y + dy:TMS.C:GROUP.y,
                  random = ~ -1  +  dm:TMS.C + dy:Stress.C + dy:TMS.C | ID, 
                  weights = varIdent(form = ~ 1 | dvnum), #separate sigma^{2}_{e} for each outcome
                  data = datalong.2.c,
                  na.action = na.exclude,
