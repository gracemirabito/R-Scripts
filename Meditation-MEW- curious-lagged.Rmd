...
MRS.data <- EMA.subset[,c("GROUP", "ID","TIME2","Time.C","Curiousity.C","PWB.C", "PhysicalActivity.C")]
#look at  data set
#head(MRS.data)
#tail(MRS.data)
...
datalong.part1 <- reshape2:: melt(data=df,
                 id.vars=c("GROUP", "ID","TIME2","Time.C","Curiousity.C","PA.lag","PWB.lag","X.M"),
...
#PART 1 restructure for just mindfulness--> rumination --> stress
MRS.data2 <- EMA.c[,c("GROUP", "ID","TIME2","Time.C","Curiousity.C","PWB.lag", "Rumination.lag","X","M","Y")]
#look at  data set
#head(MRS.data)
#tail(MRS.data)
datalong.c <- melt(data=MRS.data2,
               id.vars=c("GROUP", "ID","TIME2","Time.C","Curiousity.C","PWB.lag","Rumination.lag","X"),
               na.rm=FALSE, variable.name="dv",
               value.name="z")
#look at updated data set
#head(datalong.part1)
...
model_lme.c <- lme(fixed = z ~ -1 + 
                             dm + dm:Curiousity.C + dm:Time.C + #m as outcome
                             dy + dy:PWB.lag + dy:Curiousity.C + dy:Time.C, #y as outcome
                  random = ~ -1  +  dm:Curiousity.C + dy:PWB.lag + dy:Curiousity.C | ID, correlation= corAR1(),
                  weights = varIdent(form = ~ 1 | dvnum), #separate sigma^{2}_{e} for each outcome
                  data = datalong.c,
                  na.action = na.exclude,
                  control = lmeControl(opt = "optim", maxIter = 200, msMaxIter = 200, niterEM = 50, msMaxEval = 400))
...
model_lme.c <- lme(fixed = z ~ -1 + 
                       ...
                       # y as outcome
                       dy + dy:PWB.lag + dy:Curiousity.C + dy:Time.C +
                       #  add in group as main effect and moderator
                       dy:GROUP.y + dy:PWB.lag:GROUP.y + dy:Curiousity.C:GROUP.y,
                  random = ~ -1  +  dm:Curiousity.C + dy:PWB.lag + dy:Curiousity.C | ID, correlation= corAR1(),
                  weights = varIdent(form = ~ 1 | dvnum), #separate sigma^{2}_{e} for each outcome
                  data = datalong.2.c,
                  na.action = na.exclude,
