...
MRS.data <- EMA.subset[,c("GROUP", "ID","TIME2","Time.C","Curiousity.C","Stress.C", "PhysicalActivity.C")]
#look at  data set
#head(MRS.data)
#tail(MRS.data)
...
datalong.part1 <- reshape2:: melt(data=df,
                 id.vars=c("GROUP", "ID","TIME2","Time.C","Curiousity.C","PA.lag","Stress.lag","X.M"),
...
model_lme.c <- lme(fixed = z ~ -1 + 
                  ...
                  random = ~ -1  +  dm:Curiousity.C + dy:Stress.lag + dy:Curiousity.C | ID, correlation= corAR1(),
                  weights = varIdent(form = ~ 1 | dvnum), #separate sigma^{2}_{e} for each outcome
                  data = datalong.c,
                  na.action = na.exclude,
                  control = lmeControl(opt = "optim", maxIter = 200, msMaxIter = 200, niterEM = 50, msMaxEval = 400))
...
model_lme.c <- lme(fixed = z ~ -1 + 
                       ...
                       #  add in group as main effect and moderator
                       dy:GROUP.y + dy:Stress.lag:GROUP.y + dy:Curiousity.C:GROUP.y,correlation= corAR1(),
