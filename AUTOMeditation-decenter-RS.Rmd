...
EMA.subset$Y.S <- EMA.subset$Stress.C
...
DecRS.data <- EMA.subset[,c("GROUP", "ID","TIME2","Time.C","Decentering.C","Stress.C", "Rumination.C","X.M","M.R","Y.S")]
#look at  data set
#head(MRS.data)
#tail(MRS.data)
datalong.part1 <- melt(data=DecRS.data,
               id.vars=c("GROUP", "ID","TIME2","Time.C","Decentering.C","Stress.C","Rumination.C","X.M"),
...
datalong.part1$dy <- ifelse(datalong.part1$dv=="Y.S", 1, 0)
...
```{r}
# pull out person-specific coefficients NEED TO RUN THE MODEL_LME.PART1 NO MODERATION FIRST !!!!!!!
...
model_lme.part1 <- lme(fixed = z ~ -1 + 
                             ...
                             dy + dy:Rumination.C + dy:Decentering.C + dy:Time.C, #y as outcome
                  random = ~ -1  +  dm:Decentering.C + dy:Rumination.C + dy:Decentering.C | ID, correlation= corAR1(),
...
Effects on Y (Stress):
	0.22 = effect of m –> y (b: Rumination predicting Stress) ***
	0.002 = effect of x –> y (c’: Mindfulness predicting Stress, after adjusting for Rumination) NOT SIG 
	-0.001 = effect of time –> y (time trend in Stress) NOT SIG 
...
sig2_ey <- (1.708532 *as.numeric(VarCorr(model_lme.part1)["Residual", "StdDev"]))^2
sig2_ey
...
model_lme_mod.part1 <- lme(fixed = z ~ -1 + 
                       ...
                       # y as outcome
                       dy + dy:Rumination.C + dy:Decentering.C + dy:Time.C +
                       #  add in group as main effect and moderator
                       dy:GROUP.y + dy:Rumination.C:GROUP.y + dy:Decentering.C:GROUP.y,
                  random = ~ -1  +  dm:Decentering.C + dy:Rumination.C + dy:Decentering.C | ID, correlation= corAR1(),
                  weights = varIdent(form = ~ 1 | dvnum), #separate sigma^{2}_{e} for each outcome
...
#PART 1 restructure for just mindfulness--> rumination --> stress
MRS.data <- EMA.c[,c("GROUP", "ID","TIME2","Time.C","Decentering.C","Stress.C", "Rumination.C","X","M","Y")]
#look at  data set
#head(MRS.data)
#tail(MRS.data)
datalong.c <- melt(data=MRS.data,
               id.vars=c("GROUP", "ID","TIME2","Time.C","Decentering.C","Stress.C","Rumination.C","X"),
               na.rm=FALSE, variable.name="dv",
               value.name="z")
#look at updated data set
#head(datalong.part1)
...
model_lme.c <- lme(fixed = z ~ -1 + 
                  ...
                  random = ~ -1  +  dm:Decentering.C + dy:Stress.C + dy:Decentering.C | ID, correlation= corAR1(), 
                  weights = varIdent(form = ~ 1 | dvnum), #separate sigma^{2}_{e} for each outcome
                  data = datalong.c,
                  na.action = na.exclude)
                  #control = lmeControl(opt = "optim", maxIter = 200, msMaxIter = 200, niterEM = 50, msMaxEval = 400))
