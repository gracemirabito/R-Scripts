...
vars <- c("TMS.C", "Curiousity.C", "Decentering.C", "Stress.C", "PWB.C", "PROMIS.C", "PhysicalActivity.C")
#descriptives
describe(EMA.subset[,vars])

#PART 1 restructure for just mindfulness--> rumination --> stress
MRS.data <- EMA.subset[,c("GROUP", "ID","TIME2","Time.C","Decentering.C","PROMIS.C", "PhysicalActivity.C")]
#look at  data set
#head(MRS.data)
#tail(MRS.data)
...
MRS.data.redo <- MRS.data1 %>%                            # Add lagged column
  group_by(ID) %>%
  dplyr::mutate(PA.lag = dplyr::lead(PhysicalActivity.C, n = 1, default = NA), Depression.lag = dplyr::lead(PROMIS.C, n = 2, default = NA)) %>% 
  as.data.frame()
MRS.data.redo 
...
model_lme.part1 <- lme(fixed = z ~ -1 + 
                             ...
                             dy + dy:PA.lag + dy:Decentering.C + dy:Time.C, #y as outcome
