...
#make x,y,m 
EMA.subset$X.M <- EMA.subset$Decentering.C
EMA.subset$M.R <- EMA.subset$PhysicalActivity.C
EMA.subset$Y.M <- EMA.subset$PROMIS.C

#variables of interest for part 1 
vars <- c("TMS.C", "Curiousity.C", "Decentering.C", "PhysicalActivity.C", "PROMIS.C", "PROMIS.C", "Stress.C")
#descriptives
describe(EMA.subset[,vars])

#PART 1 restructure for just mindfulness--> rumination --> stress
MRS.data <- EMA.subset[,c("GROUP", "ID","TIME2","Time.C","Decentering.C","PROMIS.C", "PhysicalActivity.C","X.M","M.R","Y.M")]
#look at  data set
#head(MRS.data)
#tail(MRS.data)
datalong.part1 <- melt(data=MRS.data,
               id.vars=c("GROUP", "ID","TIME2","Time.C","Decentering.C","PROMIS.C","PhysicalActivity.C","X.M"),
...
datalong.part1$dy <- ifelse(datalong.part1$dv=="Y.M", 1, 0)
...
model_lme.part1 <- lme(fixed = z ~ -1 + 
                             dm + dm:Decentering.C + dm:Time.C + #m as outcome
                             dy + dy:PhysicalActivity.C + dy:Decentering.C + dy:Time.C, #y as outcome
                  random = ~ -1  +  dm:Decentering.C + dy:PhysicalActivity.C + dy:Decentering.C | ID, 
...
#test<- glmer(z ~ -1 + dm + dm:Decentering.C + dm:Time.C + dy + dy:PhysicalActivity.C + dy:Decentering.C + dy:Time.C + (-1 +  dm:Decentering.C + dy:PhysicalActivity.C + dy:Decentering.C | ID), family= binomial, control=glmerControl(optimizer="bobyqa"), data= datalong.part1, na.action=na.exclude)
...
sig2_a <- as.numeric(VarCorr(model_lme.part1)["dm:Decentering.C", "Variance"])
sig2_a

#variance of the *b* paths
sig2_b <- as.numeric(VarCorr(model_lme.part1)["dy:PhysicalActivity.C", "Variance"])
sig2_b

#variance of the *c* paths
sig2_cprime <- as.numeric(VarCorr(model_lme.part1)["Decentering.C:dy", "Variance"])
sig2_cprime
...
covajbj <- cortocov(r = as.numeric(VarCorr(model_lme.part1)["dy:PhysicalActivity.C","Corr"]),
         var1 = as.numeric(VarCorr(model_lme.part1)["dm:Decentering.C","Variance"]),
         var2 = as.numeric(VarCorr(model_lme.part1)["dy:PhysicalActivity.C","Variance"]))
...
re.2 <- merge(re, dplyr::select(group.re, ID, GROUP), by = "ID")
#re$GROUP <- group.re$GROUP 
re.2$GROUP <- recode(re.2$GROUP, "0"=-0.5 , "1"=0.5 )
...
datalong2.part1 <- merge(datalong.part1, dplyr::select(re.2, ID, GROUP), by = "ID")
#look at updated data set
#head(datalong.part1)
#tail(datalong.part1)
...
model_lme_mod.part1 <- lme(fixed = z ~ -1 + 
                       # m as outcome
                       dm + dm:Decentering.C + dm:Time.C + 
                       # add in group as main effect and moderator
                       dm:GROUP.y + dm:Decentering.C:GROUP.y + 
                       
                       # y as outcome
                       dy + dy:PhysicalActivity.C + dy:Decentering.C + dy:Time.C +
                       #  add in group as main effect and moderator
                       dy:GROUP.y + dy:PhysicalActivity.C:GROUP.y + dy:Decentering.C:GROUP.y,
                  random = ~ -1  +  dm:Decentering.C + dy:PhysicalActivity.C + dy:Decentering.C | ID, 
                  weights = varIdent(form = ~ 1 | dvnum), #separate sigma^{2}_{e} for each outcome
...
#### Effects on M (PhysicalActivity.C):

* **`r round(lme4::fixef(model_lme_mod.part1)["dm:Decentering.C"], 3)` = effect of x --> m (*a*: work stressors predicting work dissatisfaction across averaging conditions)**     
...
* **`r round(lme4::fixef(model_lme_mod.part1)["dm:Decentering.C:GROUP.y"], 3)` = interaction of x and treatment --> m (moderation of *a*: differences in effect of work stressors predicting work dissatisfaction as a function of treatment )** 

#### Effects on Y (Stress.C):

* **`r round(lme4::fixef(model_lme_mod.part1)["dy:PhysicalActivity.C"], 3)` = effect of m --> y (*b*: work dissatisfaction predicting relationship dissatisfaction averaging across conditions)**    

* **`r round(lme4::fixef(model_lme_mod.part1)["Decentering.C:dy"], 3)` = effect of x --> y (*c'*: work stressors predicting relationship dissatisfaction, after adjusting for work dissatisfaction, averaging across conditions)** 
...
* **`r round(lme4::fixef(model_lme_mod.part1)["Decentering.C:GROUP.y:dy"], 3)` = interaction of x and treatment --> y (moderation of *c*: differences in effect of work stressors predicting relationship dissatisfaction as a function of treatment)** 

* **`r round(lme4::fixef(model_lme_mod.part1)["GROUP.y:dy:PhysicalActivity.C"], 3)` = interaction of m and treatment --> y (moderation of *b*: differences in effect of work dissatisfaction predicting relationship dissatisfaction as a function of treatment)** 
...
# mediation paths
a_mod <- as.numeric(lme4::fixef(model_lme_mod.part1)["dm:Decentering.C"])
a_mod
b_mod <- as.numeric(lme4::fixef(model_lme_mod.part1)["dy:PhysicalActivity.C"])
b_mod
cprime_mod <- as.numeric(lme4::fixef(model_lme_mod.part1)["Decentering.C:dy"])
cprime_mod

# moderation of mediation paths
aXtreat_mod <- as.numeric(lme4::fixef(model_lme_mod.part1)["dm:Decentering.C:GROUP.y"])
aXtreat_mod
bXtreat_mod <- as.numeric(lme4::fixef(model_lme_mod.part1)["GROUP.y:dy:PhysicalActivity.C"])
bXtreat_mod
cprimeXtreat_mod <- as.numeric(lme4::fixef(model_lme_mod.part1)["Decentering.C:GROUP.y:dy"])
cprimeXtreat_mod
```
...
covajbj_mod <- cortocov(r = as.numeric(VarCorr(model_lme_mod.part1)["dy:PhysicalActivity.C","Corr"]),
         var1 = as.numeric(VarCorr(model_lme_mod.part1)["dm:Decentering.C","Variance"]),
         var2 = as.numeric(VarCorr(model_lme_mod.part1)["dy:PhysicalActivity.C","Variance"]))
covajbj_mod
```
...
ind_ranefs_treat <- merge(ind_ranefs, dplyr::select(re.2, ID, GROUP), by = "ID")


# compute person specific indirect effect
ind_ranefs_treat$indirect <-
  # moderated a path
  (ind_ranefs_treat$dm.Decentering.C + ind_ranefs_treat$GROUP * ind_ranefs_treat$dm.Decentering.C.GROUP.y) *
  # moderated b path
  (ind_ranefs_treat$dy.PhysicalActivity.C + ind_ranefs_treat$GROUP * ind_ranefs_treat$GROUP.y.dy.PhysicalActivity.C) 
...
#make x,y,m 
EMA.c <- EMA.subset
EMA.c$X <- EMA.c$Decentering.C
EMA.c$M <- EMA.c$PROMIS.C
EMA.c$Y <- EMA.c$PhysicalActivity.C

#variables of interest for part 1 
vars <- c("TMS.C", "Decentering.C", "Curiousity.C", "Stress.C", "PWB.C", "PROMIS.C", "PhysicalActivity.C")
#descriptives
describe(EMA.c[,vars])

#PART 1 restructure for just mindfulness--> rumination --> stress
MRS.data <- EMA.c[,c("GROUP", "ID","TIME2","Time.C","Decentering.C","PROMIS.C", "PhysicalActivity.C","X","M","Y")]
#look at  data set
#head(MRS.data)
#tail(MRS.data)
datalong.c <- melt(data=MRS.data,
               id.vars=c("GROUP", "ID","TIME2","Time.C","Decentering.C","PROMIS.C","PhysicalActivity.C","X"),
               na.rm=FALSE, variable.name="dv",
               value.name="z")
#look at updated data set
#head(datalong.part1)
...
model_lme.c <- lme(fixed = z ~ -1 + 
                             dm + dm:Decentering.C + dm:Time.C + #m as outcome
                             dy + dy:PROMIS.C + dy:Decentering.C + dy:Time.C, #y as outcome
                  random = ~ -1  +  dm:Decentering.C + dy:PROMIS.C + dy:Decentering.C | ID, 
                  weights = varIdent(form = ~ 1 | dvnum), #separate sigma^{2}_{e} for each outcome
                  data = datalong.c,
                  na.action = na.exclude,
                  control = lmeControl(opt = "optim", maxIter = 200, msMaxIter = 200, niterEM = 50, msMaxEval = 400))
...
model_lme.c <- lme(fixed = z ~ -1 + 
                       ...
                       # y as outcome
                       dy + dy:PROMIS.C + dy:Decentering.C + dy:Time.C +
                       #  add in group as main effect and moderator
                       dy:GROUP.y + dy:PROMIS.C:GROUP.y + dy:Decentering.C:GROUP.y,
                  random = ~ -1  +  dm:Decentering.C + dy:PROMIS.C + dy:Decentering.C | ID, 
                  weights = varIdent(form = ~ 1 | dvnum), #separate sigma^{2}_{e} for each outcome
                  data = datalong.2.c,
                  na.action = na.exclude,
