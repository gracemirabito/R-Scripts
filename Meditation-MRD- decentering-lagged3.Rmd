...
MRS.data <- EMA.subset[,c("GROUP", "ID","TIME2","Time.C","Decentering.C","PROMIS.C", "Rumination.C")]
#look at  data set
#head(MRS.data)
#tail(MRS.data)
...
MRS.data.redo <- MRS.data1 %>%                            # Add lagged column
  group_by(ID) %>%
  dplyr::mutate(Rumination.lag = dplyr::lead(Rumination.C, n = 2, default = NA), Depression.lag = dplyr::lead(PROMIS.C, n = 2, default = NA)) %>% 
  as.data.frame()
MRS.data.redo 
...
df = subset(MRS.data.redo, select = -c(Rumination.C, PROMIS.C) )


#melting the daya to use the dummy code double indicators
datalong.part1 <- reshape2:: melt(data=df,
                 id.vars=c("GROUP", "ID","TIME2","Time.C","Decentering.C","Rumination.lag","Depression.lag","X.M"),
...
model_lme.part1 <- lme(fixed = z ~ -1 + 
                             ...
                             dy + dy:Rumination.lag + dy:Decentering.C + dy:Time.C, #y as outcome
...
model_lme.c <- lme(fixed = z ~ -1 + 
                  ...
                  random = ~ -1  +  dm:Decentering.C + dy:Depression.lag + dy:Decentering.C | ID, 
                  weights = varIdent(form = ~ 1 | dvnum), #separate sigma^{2}_{e} for each outcome
                  data = datalong.2.c,
                  na.action = na.exclude,
                  control = lmeControl(opt = "optim", maxIter = 200, msMaxIter = 200, niterEM = 50, msMaxEval = 400))
